...
{
    return std::make_shared<mcl::BufferStream>(allocating_connection, server, mcl::BufferStreamMode::Consumer, client_platform, protobuf_bs, make_perf_report(logger), surface_name);
}
...
std::shared_ptr<mcl::ClientBufferStream> mcl::DefaultClientBufferStreamFactory::make_producer_stream(
    ...
    std::string const& surface_name)
{
    return std::make_shared<mcl::BufferStream>(allocating_connection, server, mcl::BufferStreamMode::Producer, client_platform, protobuf_bs, make_perf_report(logger), surface_name);
}
...
mcl::ClientBufferStream* mcl::DefaultClientBufferStreamFactory::make_producer_stream(
    MirConnection* allocating_connection,
    mp::DisplayServer& server,
    mp::BufferStreamParameters const& params,
    mir_buffer_stream_callback callback,
    void* context)
{
    return new mcl::BufferStream(allocating_connection, server, client_platform, params, make_perf_report(logger), callback, context);
}
