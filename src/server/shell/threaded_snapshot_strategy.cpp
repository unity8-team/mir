...
{
public:
    SnapshottingFunctor(std::shared_ptr<BufferPixels> const& pixels)
    ...
    void take_snapshot(WorkItem const& wi)
    {
        wi.surface_buffer_access->with_most_recent_buffer_do(
            ...
            {
                pixels->extract_from(buffer);
            });

        auto data = pixels->as_argb_8888();

        wi.snapshot_taken(Snapshot{data.size, data.stride, data.pixels});
    }
...
private:
    bool running;
    std::shared_ptr<BufferPixels> const pixels;
