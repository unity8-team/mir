...
{
    std::unique_lock<std::mutex> lock(handles_mutex);
    printf("Input surface opened: %s \n", opened_surface->name().c_str());
...
{
    std::unique_lock<std::mutex> lock(handles_mutex);
    printf("Input surface closed: %s \n", closed_surface->name().c_str());
