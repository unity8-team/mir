...
mf::ProtobufProtocol::ProtobufProtocol(std::unique_ptr<ProtobufIpcFactory> ipc_factory,
                                                 ...
                                                 std::string const&)
{
    ...
    {
        auto const message_sender =
            std::make_shared<detail::ProtobufResponder>(messenger, ipc_factory->resource_cache());

        auto const event_sink = std::make_shared<detail::EventSender>(messenger);
        auto const msg_processor = create_processor(
            message_sender, ipc_factory->make_ipc_server(authorizer, creds, event_sink, connection_context), report);

        auto const& connection =
            std::make_shared<mfd::SocketConnection>(messenger, next_id(), connections, msg_processor);
...
std::shared_ptr<mfd::MessageProcessor> mf::ProtobufProtocol::create_processor(
...
{
    return std::make_shared<detail::ProtobufMessageProcessor>(sender, display_server, report);
}
