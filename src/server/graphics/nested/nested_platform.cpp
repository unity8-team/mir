...
std::shared_ptr<mg::Display> mgn::NestedPlatform::create_display(std::shared_ptr<mg::DisplayConfigurationPolicy> const& /*initial_conf_policy*/, std::shared_ptr<mg::OutputConfiguration> const& output_config)
{
    return std::make_shared<mgn::NestedDisplay>(connection, event_handler, display_report, output_config);
}
