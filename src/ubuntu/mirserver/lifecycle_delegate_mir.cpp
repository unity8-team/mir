...
uams::LifecycleDelegate::LifecycleDelegate()
    : resumed_cb(nullptr),
      stop_cb(nullptr),
      ctx(nullptr)
{
...
UApplicationLifecycleDelegate* uams::LifecycleDelegate::as_u_lifecycle_delegate()
{
    return static_cast<UApplicationLifecycleDelegate*>(this);
}

uams::LifecycleDelegate* uams::LifecycleDelegate::from_u_lifecycle_delegate(UApplicationLifecycleDelegate *u_delegate)
{
    return static_cast<uams::LifecycleDelegate*>(u_delegate);
}

void uams::LifecycleDelegate::ref()
{
    ref_count++;
}

void uams::LifecycleDelegate::unref()
{
    ref_count--;
    if (ref_count == 0)
        delete this;
...
{
    return (new uams::LifecycleDelegate)->as_u_lifecycle_delegate();
}
    
void
u_application_lifecycle_delegate_destroy(UApplicationLifecycleDelegate *u_delegate)
{
    auto delegate = uams::LifecycleDelegate::from_u_lifecycle_delegate(u_delegate);
