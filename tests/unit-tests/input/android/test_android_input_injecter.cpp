...
#include "src/server/input/android/android_input_injecter.h"
...
TEST(AndroidInputInjecterSetup, inject_input_throw_behavior)
{
    ...
    mia::InputInjecter injecter(dispatcher, mt::fake_shared(repository));
    ...
    EXPECT_THROW({
            // We can't inject input to unregistered channels
            MirEvent ev;
            memset(&ev, 0, sizeof(MirEvent));

            injecter.inject_input(stub_channel, ev);
    }, std::logic_error);
}

TEST(AndroidInputInjecterSetup, inject_input_behavior)
{
    using namespace ::testing;
    ...
    EXPECT_CALL(*dispatcher, injectEventToWindow(stub_window_handle, _))
        .Times(1);
    EXPECT_CALL(repository, handle_for_channel(stub_channel))
        .Times(1)
