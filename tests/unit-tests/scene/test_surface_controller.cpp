...
struct MockWindowManager : public msh::WindowManager
{
...
{
    MockWindowManager window_manager;
    ...
    void SetUp()
    {
        ...
        ON_CALL(window_manager, place(_, _)).WillByDefault(ReturnArg<1>());
    }
};
}
...
TEST_F(SurfaceController, add_and_remove_surface)
{
    ...
    ms::SurfaceController controller(
        ...
        mt::fake_shared(window_manager),
        mt::fake_shared(model));

    InSequence seq;
    EXPECT_CALL(window_manager, place(_, _)).Times(1);
...
TEST_F(SurfaceController, offers_create_surface_parameters_to_window_manager)
{
    using namespace ::testing;
    ...
    EXPECT_CALL(window_manager, place(Ref(session), Ref(params))).Times(1)
        .WillOnce(Return(ms::a_surface()));

    controller.add_surface(params, &session);
}

TEST_F(SurfaceController, forwards_create_surface_parameters_from_window_manager_to_underlying_factory)
{
    using namespace ::testing;
    EXPECT_CALL(mock_surface, add_observer(_)).Times(AnyNumber());
    EXPECT_CALL(model, add_surface(_,_,_)).Times(AnyNumber());
    ...
    EXPECT_CALL(window_manager, place(_, Ref(params))).Times(1)
