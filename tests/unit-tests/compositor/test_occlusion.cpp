...
struct OcclusionFilterTest : public Test
{
    ...
    SceneElementList scene_element_list_from(
        std::vector<std::shared_ptr<mg::Renderable>> renderables)
    {
        SceneElementList elements;
...
TEST_F(OcclusionFilterTest, single_window_not_occluded)
{
    ...
    auto list = scene_element_list_from({window});
 
    filter_occlusions_from(list, monitor_rect);
    ASSERT_EQ(1u, list.size());
    EXPECT_EQ(window, list.front()->renderable());
}
...
{ // Regression test for LP: #1301115
    ...
    auto list = scene_element_list_from({left, right, top, bottom});
...
TEST_F(OcclusionFilterTest, smaller_window_occluded)
{
    ...
    auto list = scene_element_list_from({bottom, top});

    filter_occlusions_from(list, monitor_rect);

    ASSERT_EQ(1u, list.size());
    EXPECT_EQ(top, list.front()->renderable());
}
...
TEST_F(OcclusionFilterTest, translucent_window_occludes_nothing)
{
    ...
    EXPECT_EQ(bottom, list.front()->renderable());
    EXPECT_EQ(top, list.back()->renderable());
}
...
TEST_F(OcclusionFilterTest, cascaded_windows_never_occluded)
{
    ...
    auto list = scene_element_list_from(renderables);
...
TEST_F(OcclusionFilterTest, some_occluded_and_some_not)
{
    ...
    auto list = scene_element_list_from({
